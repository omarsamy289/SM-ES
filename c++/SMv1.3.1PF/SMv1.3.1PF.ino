/*
 * ╔══════════════════════════════════════════════════════════════════╗
 * ║         SMART MATTRESS — SMv1.3.1PF  (O Solutions Everest)      ║
 * ║                         Omar Samy                               ║
 * ╠══════════════════════════════════════════════════════════════════╣
 * ║  HARDWARE MAP                                                   ║
 ...
 * ║  Relay Fan RIGHT        ── GPIO 1  (TX0 — see note)            ║
 * ║  Relay Fan LEFT         ── GPIO 3  (RX0 — see note)            ║
 ...
 * ╠══════════════════════════════════════════════════════════════════╣
 * ║  RELAY NOTE: GPIO1=TX0, GPIO3=RX0. Serial.begin() must NOT     ║
 * ║  be called (or called only for debug, then relay control will   ║
 * ║  conflict). Relay is active-LOW (most relay modules).           ║
 * ║                                                                 ║
 * ║  REQUIRED LIBRARIES:                                            ║
...
#define RELAY_RIGHT     1            // GPIO1/TX0 — right ventilation fan
#define RELAY_LEFT      3            // GPIO3/RX0 — left  ventilation fan
...
TwoWire I2C_OLED = TwoWire(1);  // OLED  SDA=22 SCL=19
TwoWire I2C_MPU  = TwoWire(2);  // MPU   SDA=25 SCL=26  (software-emulated)
...
bool     buzzRunning    = false;     // suppresses OLED motor overlay
...
// ══════════════════════════════════════════════════════════════════
void setup() {
  // NOTE: Serial disabled to free GPIO1/GPIO3 for relays.
  // Re-enable for debugging only (relay control will fail while Serial is on):
  // Serial.begin(115200);
  ...
  if (mpuErr == 0) {
    ...
    // Enable motion-detection interrupt on INT pin
    // MPU6050_light doesn't wrap the INT register directly,
    // so we write it manually via I2C_MPU:
    I2C_MPU.beginTransmission(0x68);
    I2C_MPU.write(0x1F); I2C_MPU.write(10);   // MOT_THR = 10 LSB
    I2C_MPU.endTransmission();
    I2C_MPU.beginTransmission(0x68);
    I2C_MPU.write(0x20); I2C_MPU.write(1);    // MOT_DUR = 1 ms
    I2C_MPU.endTransmission();
    I2C_MPU.beginTransmission(0x68);
    I2C_MPU.write(0x38); I2C_MPU.write(0x40); // INT_ENABLE bit6 = MOT_EN
    I2C_MPU.endTransmission();
...
void drawOledPage(uint8_t screen) {
  ...
  switch (screen) {
    ...
    case OLED_SCREEN_BRAND: {
      ...
      oled.setCursor(34, 42); oled.print("SMv1.3.1PF");
  ...
  server.on("/setOledSeq", HTTP_GET, [](){
    ...
    if (server.hasArg("seq")) {
      ...
      while (pos <= (int)d.length() && idx < oledSeqLen) {
        ...
        oledDuration[idx] = max(1, d.substring(pos,comma).toInt());
        pos = comma + 1; idx++;
      }
    }
...
<div class="sub">O Solutions &nbsp;|&nbsp; Omar Samy &nbsp;|&nbsp; SMv1.3.1PF</div>
