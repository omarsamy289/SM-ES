/*
 * ╔══════════════════════════════════════════════════════════════════╗
 * ║         SMART MATTRESS — SMv1.3.5PF  (O Solutions Everest)      ║
 * ║                         Omar Samy                               ║
 * ╠══════════════════════════════════════════════════════════════════╣
 * ║  HARDWARE MAP                                                   ║
...
// ═══════════════════════════ I2C BUSES ════════════════════════════
// ESP32 only has 2 HW I2C buses. Three devices on three pin-pairs
// without rewiring is solved by using Wire1 for OLED (22,19) and
// calling Wire1.begin(25,26) just before MPU access, then restoring
// Wire1.begin(22,19) before any OLED write. RTC stays on Wire0(17,5).
// This works because:
//   • OLED (SSD1306) is write-only; Adafruit lib calls Wire1 only at display()
//   • MPU reads happen in checkMPU() which gates its own bus swap
//   • We restore OLED pins immediately after each MPU transaction
TwoWire I2C_RTC  = TwoWire(0);  // RTC  only  — SDA=17  SCL=5  (never changes)
TwoWire I2C_OLED = TwoWire(1);  // OLED+MPU   — pin-swapped around each use

// ═══════════════════════════ PERIPHERALS ══════════════════════════
DHT              dht(DHTPIN, DHTTYPE);
RTC_DS3231       rtc;
Adafruit_SSD1306 oled(SCREEN_W, SCREEN_H, &I2C_OLED, -1);
MPU6050          mpu(I2C_OLED);  // same TwoWire ref; pins swapped before use
...
void setup() {
  ...
  Serial.begin(115200);
  Serial.println("[BOOT] SMv1.3.5PF starting...");
  ...
  // Start Wire1 on OLED pins first so OLED initialises correctly
  ...
  // ── MPU6050 — pin-swap approach (no rewiring needed) ──
  // Temporarily move Wire1 to MPU pins to initialise the sensor,
  // then restore Wire1 to OLED pins before the first display() call.
  I2C_OLED.begin(MPU_SDA, MPU_SCL, 400000);   // swap to MPU pins
  byte mpuErr = mpu.begin();
  if (mpuErr == 0) {
    mpuOK = true;
    mpu.calcOffsets(true, true);   // gyro + accel calibration (~2 s)
    // Motion-detection interrupt registers (written directly)
  ...
  // Restore Wire1 to OLED pins — must happen before any oled.display()
  I2C_OLED.begin(OLED_SDA, OLED_SCL, 400000);
...
void checkMPU() {
  ...
  // ── Pin-swap: Wire1 → MPU pins ──
  I2C_OLED.begin(MPU_SDA, MPU_SCL, 400000);
  mpu.update();
  ...
  // Store latest readings into globals
  ...
  // ── Threshold: only react to real movement (>5° since last 500ms sample) ──
  if (totalDelta > 5.0f) {
    // Person detection on first ever motion
    if (!personDetected) {
      personDetected      = true;
      scoreTrackingActive = true;
      showPersonDetected  = true;
    ...
    if (scoreTrackingActive) {
      // REDUCED sensitivity: max 0.3 pts per 500ms sample (was up to 5 pts/100ms)
      // Full 360° turn on one axis ≈ totalDelta ~360, penalty ~0.3 pts
      float penalty = constrain(totalDelta * 0.001f, 0.0f, 0.3f);
      updateSleepScore(-penalty);   // negative = deduct
      Serial.printf("[MPU] delta=%.1f penalty=%.3f score=%.1f\n",
                    totalDelta, penalty, sleepScore);
    }
  } else if (scoreTrackingActive && totalDelta < 1.0f) {
    // ── Score recovery: +0.05 pts every 500 ms of stillness (max +6/min) ──
    updateSleepScore(0.05f);   // positive = recover
  }
}

// ══════════════════════════════════════════════════════════════════
//  OLED-safe display() wrapper — always ensures OLED pins are set
// ══════════════════════════════════════════════════════════════════
void oledShow() {
  // Guarantee Wire1 is on OLED pins before every display() call
  I2C_OLED.begin(OLED_SDA, OLED_SCL, 400000);
  oled.display();
}
...
void drawOledPage(uint8_t screen) {
  ...
  switch (screen) {
    ...
    case OLED_SCREEN_BRAND: {
      ...
      oled.setCursor(34, 42); oled.print("SMv1.3.5PF");
...
<div class="sub">O Solutions &nbsp;|&nbsp; Omar Samy &nbsp;|&nbsp; SMv1.3.5PF</div>
