...
// ═══════════════════════════ I2C BUSES ════════════════════════════
// Three devices, three pin-pairs, two HW buses — zero rewiring.
//
//  Wire  (bus 0) → RTC DS3231   SDA=17  SCL=5   PERMANENT — never changes
//  Wire1 (bus 1) → OLED SSD1306 SDA=22  SCL=19  PERMANENT — normal state
//  Wire1 (bus 1) → MPU6050      SDA=25  SCL=26  pin-swap only during
//                                                mpu.begin() + mpu.update(),
//                                                restored to OLED immediately after.
//
// WHY THIS WORKS:
//  • mpu.update() only does register reads — it does NOT call _wire.begin().
//    So: swap pins → read registers → restore pins. Takes < 1 ms.
//  • OLED is write-only; tolerates the brief gap perfectly.
//  • RTC on its own permanent bus — read any time, no conflicts ever.
//  • RTC addr=0x68, MPU addr=0x68 — same address but on DIFFERENT buses: safe.
TwoWire I2C_RTC  = TwoWire(0);  // Wire  bus 0 — RTC  SDA=17 SCL=5  PERMANENT
TwoWire I2C_OLED = TwoWire(1);  // Wire1 bus 1 — OLED SDA=22 SCL=19 PERMANENT

// ═══════════════════════════ PERIPHERALS ══════════════════════════
DHT              dht(DHTPIN, DHTTYPE);
RTC_DS3231       rtc;                                        // uses I2C_RTC (bus 0)
Adafruit_SSD1306 oled(SCREEN_W, SCREEN_H, &I2C_OLED, -1);  // uses I2C_OLED (bus 1)
MPU6050          mpu(I2C_OLED); // Wire1 — pins swapped 25/26 only during reads
