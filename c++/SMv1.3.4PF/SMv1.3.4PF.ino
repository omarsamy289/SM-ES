/*
 * ╔══════════════════════════════════════════════════════════════════╗
 * ║         SMART MATTRESS — SMv1.3.3PF  (O Solutions Everest)      ║
 * ║                         Omar Samy                               ║
 * ╠══════════════════════════════════════════════════════════════════╣
 * ║  HARDWARE MAP                                                   ║
 ...
 * ║  Relay Fan RIGHT        ── GPIO 15 (safe, no boot conflict)     ║
 * ║  Relay Fan LEFT         ── GPIO 16 (safe, no boot conflict)     ║
 * ║  Piezo Impact           ── GPIO 14  (ADC, EXT0 wake)           ║
 * ║  Piezo Vibration        ── GPIO 27  (ADC)                      ║
 * ║  DHT11                  ── GPIO 2                              ║
 * ║  RTC DS3231 SDA/SCL     ── GPIO 17 / 5   (Wire  bus 0)         ║
 * ║  RTC DS3231 SQW         ── GPIO 18                             ║
 * ║  OLED 0.96" SDA/SCL     ── GPIO 22 / 19  (Wire1 bus 1)         ║
 * ║  MPU6050  SDA/SCL/INT   ── GPIO 25 / 26 / 33  (Wire2 bus 2)   ║
 * ╠══════════════════════════════════════════════════════════════════╣
 * ║  RELAY NOTE: Moved from GPIO1/3 (TX0/RX0) to GPIO15/16 to      ║
 * ║  avoid boot/flash conflicts. Serial is now available for debug. ║
 ...
 * ║   • MPU6050_light  (rfetick) — clean, no I2Cdevlib dependency  ║
...
#define RTC_SDA  17
#define RTC_SCL  5
// OLED and MPU share Wire1 — both wired to GPIO25(SDA) / GPIO26(SCL)
#define SHARED_SDA 25   // OLED SDA + MPU SDA  (rewire OLED here)
#define SHARED_SCL 26   // OLED SCL + MPU SCL  (rewire OLED here)
// Legacy defines kept for reference only — OLED physically moved to 25/26
#define OLED_SDA SHARED_SDA
#define OLED_SCL SHARED_SCL
...
// ═══════════════════════════ I2C BUSES ════════════════════════════
// ESP32 Arduino core only supports TwoWire(0) and TwoWire(1).
// TwoWire(2) silently fails — that was the MPU "not found" bug.
//
// FIX: OLED and MPU6050 share Wire1 on the MPU's physical pins (25,26).
// Their I2C addresses are different (OLED=0x3C, MPU=0x68) so no conflict.
//
// REWIRING REQUIRED: Connect OLED SDA→GPIO25, OLED SCL→GPIO26
// (instead of 22/19). Both OLED and MPU now share the same GPIO25/26 bus.
// RTC stays on its own Wire0 bus (GPIO17/5) — unchanged.
TwoWire I2C_RTC  = TwoWire(0);  // RTC  SDA=17  SCL=5
TwoWire I2C_OLED = TwoWire(1);  // OLED+MPU6050 shared  SDA=25  SCL=26

// ═══════════════════════════ PERIPHERALS ══════════════════════════
DHT              dht(DHTPIN, DHTTYPE);
RTC_DS3231       rtc;
Adafruit_SSD1306 oled(SCREEN_W, SCREEN_H, &I2C_OLED, -1);
MPU6050          mpu(I2C_OLED);   // shares Wire1 with OLED — addr 0x68 vs 0x3C
...
void checkPiezo();
void checkMPU();
void updateSleepScore(float penalty);
...
void setup() {
  ...
  Serial.begin(115200);
  Serial.println("[BOOT] SMv1.3.3PF starting...");
  ...
  I2C_OLED.begin(SHARED_SDA, SHARED_SCL, 400000); // OLED + MPU share this bus
  ...
  // ── MPU6050 ── (MPU6050_light library)
  byte mpuErr = mpu.begin();
  if (mpuErr == 0) {
    mpuOK = true;
    mpu.calcOffsets(true, true);   // auto-calibrate gyro & accel (~2 s)
    // Enable motion-detection interrupt registers via shared bus
    I2C_OLED.beginTransmission(0x68);
    I2C_OLED.write(0x1F); I2C_OLED.write(10);   // MOT_THR = 10 LSB
    I2C_OLED.endTransmission();
    I2C_OLED.beginTransmission(0x68);
    I2C_OLED.write(0x20); I2C_OLED.write(1);    // MOT_DUR = 1 ms
    I2C_OLED.endTransmission();
    I2C_OLED.beginTransmission(0x68);
    I2C_OLED.write(0x38); I2C_OLED.write(0x40); // INT_ENABLE bit6 = MOT_EN
    I2C_OLED.endTransmission();
...
void loop() {
  ...
  // ── MPU6050 continuous update ──
  if (mpuOK && ms - lastMpuUpdate >= 100) {
    lastMpuUpdate = ms;
    checkMPU();
  }
...
void checkPiezo() {
  ...
  if (impact > PIEZO_THRESH || vib > PIEZO_THRESH) {
    ...
    if (scoreTrackingActive) updateSleepScore(0.5f);
  }
}

// ══════════════════════════════════════════════════════════════════
//  MPU6050 CHECK  (called every 100 ms)
// ══════════════════════════════════════════════════════════════════
void checkMPU() {
  mpu.update();
  ...
  // Significant movement threshold: > 3° change
  ...
  if (totalDelta > 3.0f) {
    ...
    if (scoreTrackingActive) {
      // Score penalty: scaled by movement magnitude
      // 2 half-turns on X axis = 1 full turn = -1 pt
      // Penalty scales with total angular displacement
      float penalty = constrain(totalDelta * 0.1f, 0.0f, 5.0f);
...
void updateSleepScore(float penalty) {
  ...
  sleepScore = constrain(sleepScore - penalty, 0.0f, 100.0f);
  if (fabsf(sleepScore - prev) > 0.05f) {
    lastScoreChange = millis();
    // Immediate history push on significant event
    scoreHistory[scoreHead] = sleepScore;
    scoreHead = (scoreHead + 1) % SCORE_HISTORY;
    if (scoreCount < SCORE_HISTORY) scoreCount++;
  }
...
void updateOLED() {
  ...
  // Person-detected flash takes priority (4 s)
  ...
  // Fan status overlay when any ventilation fan OR PWM fan is on
  // (but NOT during buzz — buzz suppresses it)
  bool ventActive = relayRight || relayLeft || (fanPWM > 0 && fanOn);
  if (ventActive && !buzzRunning) {
...
void drawOledPage(uint8_t screen) {
  ...
  switch (screen) {
    ...
    case OLED_SCREEN_BRAND: {
      ...
      oled.setCursor(34, 42); oled.print("SMv1.3.3PF");
...
<div class="sub">O Solutions &nbsp;|&nbsp; Omar Samy &nbsp;|&nbsp; SMv1.3.3PF</div>
...
// ── RTC / Sleep ──────────────────────────────────────────────────
function setRTC(){
  fetch('/setTime?h='+document.getElementById('th').value+
        '&m='+document.getElementById('tm').value+
        '&s='+(document.getElementById('ts').value||0))
    .then(()=>alert('RTC time updated!'));
}
function sleepNow(){
