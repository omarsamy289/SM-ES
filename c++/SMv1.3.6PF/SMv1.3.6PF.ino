/*
 * ╔══════════════════════════════════════════════════════════════════╗
 * ║         SMART MATTRESS — SMv1.3.6PF  (O Solutions Everest)      ║
 * ║                         Omar Samy                               ║
 * ╠══════════════════════════════════════════════════════════════════╣
 * ║  HARDWARE MAP                                                   ║
 ...
 * ║  DHT11                  ── GPIO 2                              ║
 * ║  RTC DS3231 SDA/SCL     ── GPIO 17 / 5   (Wire0)              ║
 * ║  RTC DS3231 SQW         ── GPIO 18                             ║
 * ║  OLED 0.96" SDA/SCL     ── GPIO 22 / 19  (Wire1 — NO REWRITE) ║
 * ║  MPU6050  SDA/SCL/INT   ── GPIO 25 / 26 / 33                  ║
 * ╠══════════════════════════════════════════════════════════════════╣
 * ║  I2C STRATEGY (no rewiring):                                   ║
 * ║   Wire0 (TwoWire 0) → RTC  only  on GPIO17/5                  ║
 * ║   Wire1 (TwoWire 1) → OLED on GPIO22/19 normally              ║
 * ║                      → swapped to GPIO25/26 only during MPU   ║
 * ║                        read, then immediately restored.        ║
 * ║  This works because SSD1306 is write-only; OLED retains its   ║
 * ║  frame buffer between display() calls.                         ║
 * ╠══════════════════════════════════════════════════════════════════╣
 * ║  REQUIRED LIBRARIES:                                           ║
...
// ═══════════════════════════ I2C BUSES ════════════════════════════
// Three devices, three pin-pairs, only two HW I2C buses — no rewiring.
//
// SOLUTION (guaranteed — no conflicts):
//   Wire  (bus 0) → MPU6050  only    SDA=25  SCL=26
//     Wire.setPins(25,26) before Wire.begin() so MPU6050_light's
//     internal _wire.begin() call stays locked to GPIO25/26.
//
//   Wire1 (bus 1) → OLED             SDA=22  SCL=19  (permanent)
//
//   Wire1 (bus 1) → RTC DS3231       SDA=17  SCL=5
//     RTC is read once per second. Wire1 is briefly swapped to 17/5,
//     RTC read happens, Wire1 restored to 22/19 immediately after.
//     OLED is write-only so it tolerates this 1 ms swap perfectly.
//
// MPU address=0x68, RTC address=0x68 — MUST be on separate buses (they are).
// OLED address=0x3C — no conflict with RTC on the swapped Wire1.
TwoWire I2C_OLED = TwoWire(1);  // Wire1 — OLED 22/19 (normal state)
// Wire (global, bus 0) used directly for MPU — configured in setup()

// ═══════════════════════════ PERIPHERALS ══════════════════════════
DHT              dht(DHTPIN, DHTTYPE);
RTC_DS3231       rtc;                                        // Wire1 swapped to 17/5
Adafruit_SSD1306 oled(SCREEN_W, SCREEN_H, &I2C_OLED, -1);  // Wire1 on 22/19
MPU6050          mpu(Wire);  // global Wire bus 0 — pins 25/26 set in setup()
...
void setup() {
  ...
  Serial.begin(115200);
  Serial.println("[BOOT] SMv1.3.6PF starting...");
  ...
  // ── I2C buses ──
  // Bus 0 (Wire) → MPU6050 on GPIO25/26
  // setPins MUST come before begin() so the library's internal begin() uses them
  Wire.setPins(MPU_SDA, MPU_SCL);
  Wire.begin();                          // 400 kHz default on ESP32

  // Bus 1 (Wire1 / I2C_OLED) → OLED on GPIO22/19
  I2C_OLED.begin(OLED_SDA, OLED_SCL, 400000);

  // ── RTC — brief Wire1 swap to 17/5 ──
  I2C_OLED.begin(RTC_SDA, RTC_SCL, 100000);
  if (rtc.begin(&I2C_OLED)) {
  ...
  // Restore Wire1 → OLED pins immediately
  I2C_OLED.begin(OLED_SDA, OLED_SCL, 400000);
  ...
  // ── MPU6050 — Wire (bus 0) permanently on GPIO25/26 ──
  // Wire.setPins(25,26) + Wire.begin() already called above.
  // MPU6050_light's internal begin() calls _wire.begin() with no args,
  // which on ESP32 re-uses the already-configured pins — so it stays on 25/26.
  byte mpuErr = mpu.begin();
  if (mpuErr == 0) {
    mpuOK = true;
    mpu.calcOffsets(true, true);   // auto-calibrate gyro + accel (~2 s)
    // Write motion-detection interrupt registers directly
    Wire.beginTransmission(0x68);
    Wire.write(0x1F); Wire.write(15);   // MOT_THR = 15 LSB
    Wire.endTransmission();
    Wire.beginTransmission(0x68);
    Wire.write(0x20); Wire.write(2);    // MOT_DUR = 2 ms
    Wire.endTransmission();
    Wire.beginTransmission(0x68);
    Wire.write(0x38); Wire.write(0x40); // INT_ENABLE: motion bit
    Wire.endTransmission();
...
//  Wire (bus 0) is permanently on GPIO25/26 — no pin-swap needed.
// ══════════════════════════════════════════════════════════════════
void checkMPU() {
  if (!mpuOK) return;
...
//  Wire1 (I2C_OLED) is permanently on OLED pins 22/19.
//  The only time it swaps is during RTC reads (once/second, <1 ms).
// ══════════════════════════════════════════════════════════════════
void oledShow() {
  I2C_OLED.begin(OLED_SDA, OLED_SCL, 400000); // ensure pins correct after any RTC swap
  oled.display();
}
...
void drawOledPage(uint8_t screen) {
  ...
  switch (screen) {
    ...
    case OLED_SCREEN_BRAND: {
      ...
      oled.setCursor(34, 42); oled.print("SMv1.3.6PF");
...
<div class="sub">O Solutions &nbsp;|&nbsp; Omar Samy &nbsp;|&nbsp; SMv1.3.6PF</div>
